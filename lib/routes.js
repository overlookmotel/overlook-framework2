...
BaseAction.prototype.init = function() {
    ...
    if (!this.view) {
        ...
        if (this.overlook.views[treePath]) {
            this.view = treePath;
        } else {
            this.view = '_default/default';
        }
    }

    // check view exists
    if (!this.overlook.views[this.view]) throw new Errors.Base("View '" + this.view + "' specified for " + this.treePath + ' does not exist');
...
BaseAction.prototype.render = function(locals) {
    var res = this.res;
    return Promise.promisify(res.render, res)(this.view + '.' + this.overlook.views[this.view], locals || {})
    .then(function(html) {
        res.send(html);
    });
};
